...
class HotArticle {
  ...
  async hkTechnology(coon) {
    ...
    const list = await page.evaluate(() => {
      ...
      for (let i = 0; i < hotList.length; i++) {
        let obj = {};
        const eleTitle = hotList[i].querySelector('.jvqc0e-7 a div');
        const eleUrl = hotList[i].querySelector('.jvqc0e-7 a');
        const eleImageUrl = hotList[i].querySelector('.s1n40ifc-0');
        const eleCreateDate = hotList[i].querySelector('.jvqc0e-13');
        const eleTypeSub = hotList[i].querySelector('.jvqc0e-12 a');
        let title = '';
        let url = '';
        let imageUrl = '';
        let createDate = '';
